...
Faze leta unapred i leta unazad se smenjuju {\em NC} puta, dok svaka pčela ne izgradi sopstveno rešenje ili ne uradi izmenu rešenja. {\em NC} je predstavlja parametar koji služi za definisanje učestalosti komunikacije između pčela. Kada se izvrši {\em NC} iteracija/koraka, određuje se najbolje rešenje od svih mogućih {\em B} rešenja. To rešenje se posle koristi pri ažuriranju globalno najboljeg rešenja. Ovime je iteracija algoritma gotova i brišu se svih {\em B} rešenja, te započinje nova iteracija.
...
Lojalnost pčele zavisi od formule (*), kao i generatora slučajnih brojeva. Ako je slučajni broj manji od izračunate vrednosti, pčela ostaje pri svom rešenju. U suprotnom, neopredeljena je.
...
$p_b = \frac{Q_b}{\sum_{k=1}^{RE}(Q_k)},	b = 1, 2, . . . RE$ \\

Gde je {\em Qk} normalizovana vrednost funkcije cilja k-tog reklamiranog rešenja. {\em RE} je broj regrutera. Svaka neopredeljena pčela se pridružuje jednom od regrutera preko ruleta.
